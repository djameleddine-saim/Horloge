...
def time():
    if format.get() == "AM/PM":
        string = strftime('%I:%M:%S PM')
    else:
        string = strftime('%H:%M:%S AM')
...
def set_alarm():
    ...
    if format.get() == "AM/PM":
        alarm_time = alarm_time + " " + am_pm.get()
    alarm_set = True
...
def set_time():
    global current_time
    current_time = set_time_entry.get() + " " + set_am_pm.get()
...
set_am_pm = Combobox(set_time_frame, values=["AM", "PM"])
set_am_pm.pack(side=LEFT)
...
toggle_format_button = Button(root, text="AM/PM", command=toggle_format)
...
am_pm = Combobox(alarm_frame, values=["AM", "PM"])
